...
# One weak-to-strong run: weak probe, activations, selection scores, banding, LoRA
# fine-tune, evaluation. Every knob is an environment variable.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT_DIR"

# Activate the Python environment so `python3` has the ML stack (torch/transformers/peft):
# set W2S_ENV_ACTIVATE to an activate script, or keep a repo-local .venv.
...
# Weak and strong should share a FAMILY / TOKENIZER: the excess-loss selectors
# compare the two models' per-option distributions, which is only clean when the
...
OUTPUT_DIR="${OUTPUT_DIR:-results/w2s_lora/seed42}"
...
# EPOCHS>0 trains each run for that many passes over its subset; EPOCHS=0 uses the MAX_TRAIN_STEPS cap.
...
RUNS="${RUNS:-base,ground_truth,weak_label,middle_balanced}"
...
# weight decay of the step-1 MLP behind mlpstep1 (RADS); empty keeps the .py default of 30
if [[ -n "${MLPSTEP1_WD:-}" ]]; then
  EXTRA_ARGS+=(--mlpstep1-wd "$MLPSTEP1_WD")
fi
# batch size of the yes/no test evaluations; empty keeps the training batch (the .py default)
...
if [[ "${EVAL_3CLASS:-1}" == "1" ]]; then
  EXTRA_ARGS+=(--eval-3class --n-eval-questions "${N_EVAL_QUESTIONS:-$N_TEST}")
fi
...
# Unset WANDB_PROJECT means no wandb call at all, so a sweep behaves exactly as
...
python3 scripts/run_w2s_lora.py \
  --dataset "$DATASET" \
