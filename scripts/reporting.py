...
def format_summary(dataset: str, sciq_use_support: bool, anli_round: str) -> dict[str, str]:
    ...
    if dataset == "wanli":
        return {
            ...
            "takeaway": "This run uses WANLI (worker-and-AI adversarial NLI) as binary candidate-relation correctness.",
        }
    if dataset == "fevernli":
        return {
            ...
            "takeaway": "This run uses FEVER recast as 3-way fact-verification NLI (candidate-verdict correctness).",
        }
    if dataset == "imppres":
        return {
            ...
            "takeaway": "This run uses ImpPres presupposition NLI (templated, structurally adversarial).",
        }
    if dataset == "aquarat":
        return {
            ...
            "takeaway": "This run uses AQuA-RAT (5-option math word problems).",
        }
    if dataset == "quail":
        return {
            ...
            "label": "1 if the candidate is the correct option, else 0",
            "takeaway": "This run uses QuAIL (4-option multi-domain RC).",
        }
    if dataset == "riddlesense":
        return {
            ...
            "label": "1 if the candidate is the correct option, else 0",
            "takeaway": "This run uses RiddleSense (5-option human riddles).",
        }
    if dataset == "scinli":
        return {
            ...
            "takeaway": "This run uses SciNLI (4-way scientific-paper NLI, 107k, human-written).",
        }
    if dataset in ("agnews", "imdb"):
        names = {"agnews": "AG News (4-way news-topic classification)",
                 "imdb": "IMDb (2-way review sentiment)"}
        return {
            "task": f"paper-style binary candidate-answer correctness ({names[dataset]})",
            "candidate_text": "text block + question + candidate option",
            "label": "1 if the candidate option is the correct one, else 0",
            "takeaway": f"This run uses {names[dataset]}.",
        }
    if dataset == "qnli":
        return {
            "task": "paper-style binary candidate-answer correctness (QNLI (QA-as-NLI, GLUE))",
            "candidate_text": "sentence-pair block + question + candidate option",
            "label": "1 if the candidate option is the correct one, else 0",
            "takeaway": "This run uses QNLI (QA-as-NLI, GLUE).",
        }
    if dataset in ("tweetsent", "wikitoxic", "scitail", "boolq", "ethicsjustice"):
        names = {"tweetsent": "TweetEval sentiment (3-way tweet sentiment, SemEval-2017 Task 4A)",
                 "wikitoxic": "Wiki Toxic (2-way toxicity of Wikipedia talk-page comments, Jigsaw; classes cut to equal size)",
                 "scitail": "SciTail (2-way science entailment)",
                 "boolq": "BoolQ (yes/no questions over a passage)",
                 "ethicsjustice": "ETHICS justice (2-way reasonableness of a claim of desert or impartiality)"}
        return {
            "task": f"paper-style binary candidate-answer correctness ({names[dataset]})",
            "candidate_text": "text block + question + candidate option",
            "label": "1 if the candidate option is the correct one, else 0",
    ...
    if dataset in ("blimp", "sst2"):
        names = {"blimp": "BLiMP (2-option grammaticality minimal pairs, 67 paradigms pooled)",
                 "sst2": "SST-2 (binary sentiment, GLUE)"}
        return {
            "task": f"paper-style binary candidate-answer correctness ({names[dataset]})",
            "candidate_text": "sentence block + question + candidate option",
            "label": "1 if the candidate option is the correct one, else 0",
            "takeaway": f"This run uses {names[dataset]}.",
        }
    if dataset == "cosmosqa":
        return {
            "task": "paper-style binary candidate-answer correctness (CosmosQA (4-option commonsense reading comprehension))",
            "candidate_text": "context/sentence block + 'Q: ... A: {candidate}'",
            "label": "1 if the candidate is the correct option, else 0",
            "takeaway": "This run uses CosmosQA (4-option commonsense reading comprehension).",
        }
    if dataset == "csqa":
        return {
            "task": "paper-style binary candidate-answer correctness (CommonsenseQA)",
            "candidate_text": "'Q: {question} A: {candidate}'",
            "label": "1 if the candidate is the correct option, else 0",
            "takeaway": "This run uses CommonsenseQA (5-option commonsense MC).",
        }
